  ...
  prompting() {
    ...
    const serverQuestions = [
      ...
      },
      {
        ...
        message:
          '[Provisioning/Deployment](Optionnal) Your production IP address',
        default: '',
        when: answers => {
    ...
    return this.prompt(serverQuestions).then(serverAnswers => {
      this.provisioningVars = {
        appName: this.appname,
        repositoryUrl:
          serverAnswers['repository-url'] || this.options['repository-url'],
        databaseHost: 'localhost',
        stagingIpAddress:
          serverAnswers['staging-ip-address'] ||
          this.options['staging-ip-address'],
        prodIpAddress:
          serverAnswers['prod-ip-address'] || this.options['prod-ip-address'],
        prodDatabasePassword:
          serverAnswers['prod-database-password'] ||
          this.options['prod-database-password'],
        stagingDatabasePassword:
          serverAnswers['staging-database-password'] ||
          this.options['staging-database-password'],
      };
    });
  }

  _addConfigurationTemplates() {
    const files = ['gitignore', '.editorconfig', 'ansible.cfg', 'Vagrantfile'];
